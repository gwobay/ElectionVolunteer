...
/*        try
	...
	public byte[] readStreamData()
	{
		...
		if (needToRead)
		{
			...
			if (left_length > 0) tmp=merge2(readLeft, tmp1, left_length, i);
			else tmp=tmp1;
		...
		if (i <= iLen+4)
		{
			...
			else
			{
				...
				System.arraycopy(tmp, 0, dBuf, 0, i);
				tmp=null;
			}
			//tmp1=null;
			byte[] more=new byte[iLen-i+4];
			while (iLen+4 > i)
			{
				...
				if (iLen + 4 >= i+i2)
				{
					System.arraycopy(more, 0, dBuf, i, i2);
					i += i2;
				}
				else
				{
					left_length=(i+i2)-(iLen+4);
					readLeft=new byte[left_length];
					//System.arraycopy(org, 0, tmp, 0, i);
					System.arraycopy(more, 0, dBuf, i, (iLen+4)-i);
					more=null;
					break;
				}
			}
	...
	public boolean sendSocketText(String text)
	{
		...
		byte[] outB=new byte[2+b.length];
		outB[0]=(byte) (b.length/127);
		outB[1]=(byte) (b.length %127);
		System.arraycopy(b, 0, outB, 2, b.length);
		return sendRawByte(outB, 0, outB.length);
	}
